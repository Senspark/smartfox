...
// LogEventType
// -------------------------------------------------------------------
boost::shared_ptr<std::string> LoggerEvent::LogEventType(LogLevel level)
{
	char buffer[100];
...
// ToString
// -------------------------------------------------------------------
boost::shared_ptr<std::string> LoggerEvent::ToString()
{
	char buffer[100];
	sprintf (buffer, "LoggerEvent %s", type->c_str());

	return boost::shared_ptr<std::string>(new string (buffer));
}
