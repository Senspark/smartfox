...
void ConfigLoader::LoadConfig(const std::string& filePath) {
    ...
    boost::shared_ptr<std::string> fileTextAsString(
        new string(fileText.get(), fileText.get() + length));
...
ConfigLoader::GetNodeText(boost::shared_ptr<XMLNode> rootNode,
    ...
    if (iterator == rootNode->end()) {
        ...
        boost::shared_ptr<XMLNodeList> xmlNodeList =
            (boost::static_pointer_cast<XMLNodeList>)(iterator->second
                                                          .XMLObjectInstance);

        list<boost::shared_ptr<XMLNode>>::iterator iteratorNodeList =
            xmlNodeList->begin();
        std::advance(iteratorNodeList, 0);
        boost::shared_ptr<XMLNode> xmlNode = *iteratorNodeList;
        boost::shared_ptr<XMLNodeValue> xmlValue =
            (boost::static_pointer_cast<XMLNodeValue>)(xmlNode->at("_text")
                                                           .XMLObjectInstance);
        return (boost::shared_ptr<std::string>)xmlValue;
    }
}
...
void ConfigLoader::TryParse() {
    ...
    try {
        ...
        map<string, tagXMLObject>::iterator iterator =
            rootNode->find("SmartFoxConfig");
        if (iterator == rootNode->end()) {
            boost::throw_exception(boost::enable_error_info(
                std::runtime_error("SmartFoxConfig tag not found")));
        }

        boost::shared_ptr<XMLNodeList> nodeList =
            (boost::static_pointer_cast<XMLNodeList>)(iterator->second
                                                          .XMLObjectInstance);
        ...
        cfgData->Debug(false);
        if ((GetNodeText(configNode, "debug") != NULL) &&
            ((*(GetNodeText(configNode, "debug"))) == "true")) {
            cfgData->Debug(true);
        }

        cfgData->UseBlueBox(false);
        if ((GetNodeText(configNode, "useBlueBox") != NULL) &&
            ((*(GetNodeText(configNode, "useBlueBox"))) == "true")) {
            cfgData->UseBlueBox(true);
        }

        if ((GetNodeText(configNode, "httpPort") != NULL) &&
            ((*(GetNodeText(configNode, "httpPort"))) != "")) {
            cfgData->HttpPort(
                atoi(GetNodeText(configNode, "httpPort")->c_str()));
        }

        if ((GetNodeText(configNode, "httpsPort") != NULL) &&
            ((*(GetNodeText(configNode, "httpsPort"))) != "")) {
            cfgData->HttpsPort(
                atoi(GetNodeText(configNode, "httpsPort")->c_str()));
        }

        if ((GetNodeText(configNode, "blueBoxPollingRate") != NULL) &&
            ((*(GetNodeText(configNode, "blueBoxPollingRate"))) != "")) {
            cfgData->BlueBoxPollingRate(
                atoi(GetNodeText(configNode, "blueBoxPollingRate")->c_str()));
        }

        cfgData->ForceIPv6(false);
        if ((GetNodeText(configNode, "forceIPv6") != NULL) &&
            ((*(GetNodeText(configNode, "forceIPv6"))) == "true")) {
        ...
        boost::shared_ptr<std::string> message(new std::string(
            "Error parsing config file: " + (*messageException)));
    ...
    boost::shared_ptr<SFSEvent> sfsEvt(
...
// OnConfigLoadFailure
// -------------------------------------------------------------------
void ConfigLoader::OnConfigLoadFailure(
    ...
    boost::shared_ptr<SFSEvent> sfsEvt(
        new SFSEvent(SFSEvent::CONFIG_LOAD_FAILURE, data));
    dispatcher->DispatchEvent(sfsEvt);
}
...
// AddEventListener
// -------------------------------------------------------------------
void ConfigLoader::AddEventListener(
