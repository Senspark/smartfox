...
namespace Requests {
	...
	public:
		...
		///					boost::shared_ptr<std::vector<boost::shared_ptr<Room>>> ptrRooms = ((boost::static_pointer_cast<vectorboost::shared_ptr<Room>>>)(ptrEventParamValueRooms));
		///					string* message = new std::string("Subscription done");
		///				}
		///
		...
		///					string* message = new std::string("Subscription failure: " +  (*ptrErrorMessage));
		///				}
		/// 	\endcode
		/// </example>
		...
		SubscribeRoomGroupRequest(const std::string& groupId);
