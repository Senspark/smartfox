...
namespace Requests {
	...
	public:
		...
		RoomSettings(const std::string& name);
		...
		boost::shared_ptr<std::vector<boost::shared_ptr<RoomVariable> > > Variables();

		/// <summary>
		/// A list of Room Variables that should be attached to the newly created Room
		/// </summary>
		/// <seealso cref="SFSRoomVariable"/>
		void Variables(boost::shared_ptr<std::vector<boost::shared_ptr<RoomVariable> > > value);
		...
		/// The id of the Room Group in which the new Room should be created
		/// </summary>
		void GroupId(const std::string& value);
