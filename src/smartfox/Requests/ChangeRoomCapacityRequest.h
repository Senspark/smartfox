...
/// </summary>
/// <remarks>
/// This request might fail if the Room was configured not be re-sizable or if
/// the User sending the request is not the owner of the Room. Moderators and
/// Administrator can override this last constraint. <para/> It is possible to
/// "shrink" the Room capacity so that maxUsers < userCount. <br/> In this case
/// nothing will happen to the "extra" users.<br/> As soon as clients will leave
/// the Room the userCount will get down to the new maxUsers value.
/// </remarks>
...
public:
    ...
    /// the new maxUsers value (use -1 if don't want to change the current
    /// value)
    /// </param>
    /// <param name="newMaxSpect">
    /// the new maxSpect value (use -1 if don't want to change the current
    /// value)
    /// </param>
    /// <example>
    ///        \code{.cpp}
    ///                ptrSmartFox->AddEventListener(SFSEvent::ROOM_CAPACITY_CHANGE,
    /// boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnRoomCapacityChange, (unsigned long
    /// long)this));
    ///                ptrSmartFox->AddEventListener(SFSEvent::ROOM_CAPACITY_CHANGE_ERROR,
    /// boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnRoomCapacityChangeError, (unsigned
    /// long long)this));
    ///
    ///             boost::shared_ptr<Room> theRoom =
    /// ptrSmartFox->GetRoomByName("DeveloperChatRoom");
    ///
    ///             // Resize the room to have 100 users and zero spectators
    ///                boost::shared_ptr<IRequest> request (new
    /// ChangeRoomCapacityRequest(theRoom, 100, 0));
    ///                ptrSmartFox->Send(request);
    ///
    ///                static void OnRoomCapacityChange(unsigned long long
    ///                ptrContext,
    /// boost::shared_ptr<BaseEvent> ptrEvent);                 static void
    /// OnRoomCapacityChangeError(unsigned long long ptrContext,
    /// boost::shared_ptr<BaseEvent> ptrEvent);
    ///
    ///                void CMyClass::OnRoomCapacityChange(unsigned long long
    ///                ptrContext,
    /// boost::shared_ptr<BaseEvent> ptrEvent);
    ///                {
    ...
    ///                }
    ///
    ///                void CMyClass::OnRoomCapacityChangeError(unsigned long
    ///                long
    /// ptrContext, boost::shared_ptr<BaseEvent> ptrEvent);
    ///                {
    ///                    CMyClass* ptrMyClass = (CMyClass*)ptrContext;
    ///
    ...
    explicit ChangeRoomCapacityRequest(const boost::shared_ptr<Room>& room,
    ...
    static boost::shared_ptr<std::string> KEY_USER_SIZE;
    static boost::shared_ptr<std::string> KEY_SPEC_SIZE;
