...
namespace Requests {
	...
	public:
		...
		/// <seealso cref="RoomSettings"/>
		/// <seealso cref="CreateRoomRequest"/>
		RoomExtension(const std::string& id, const std::string& className);
		...
		/// The fully qualified name of the main class in the extension
		/// </summary>
		boost::shared_ptr<std::string> ClassName();

		/// <summary>
		/// An optional properties file that will be loaded on the server side when the extension is initialized
		/// </summary>
		boost::shared_ptr<std::string> PropertiesFile();

		/// <summary>
		/// An optional properties file that will be loaded on the server side when the extension is initialized
		/// </summary>
		void PropertiesFile(const std::string& value);
	...
	private:
		...
		// Private members
		// -------------------------------------------------------------------
		boost::shared_ptr<std::string> id;					// <-- mandatory
		boost::shared_ptr<std::string> className;			// <-- mandatory
		boost::shared_ptr<std::string> propertiesFile;		// <-- optional
	};
