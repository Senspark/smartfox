...
/// </summary>
/// <remarks>
/// Private Messages can be exchanged between Users in the same Rooms or in
/// different Rooms.<br/> They can also be sent between Users that are not
/// joined to any Rooms at all.
/// </remarks>
...
public:
    ...
    /// <example>
    ///        \code{.cpp}
    ///                ptrSmartFox->AddEventListener(SFSEvent::PRIVATE_MESSAGE,
    ///boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnPrivateMessage, (unsigned long
    ///long)this));
    ///
    ...
    ///                boost::shared_ptr<IRequest> request (new PrivateMessageRequest("Hello
    ///Piggy!", messageRecipient->Id()));
    ///                ptrSmartFox->Send(request);
    ///
    ///                static void OnPrivateMessage(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);
    ///
    ///                void CMyClass::OnPrivateMessage(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);
    ///                {
    ...
    ///((boost::static_pointer_cast<string&lg;)(ptrEventParamValueMessage));
    ///                    string* message = new std::string("Got PM: " +
    ...
    PrivateMessageRequest(const std::string& message, int recipientId,
                          boost::shared_ptr<ISFSObject> parameters);

    /// <summary>
    /// <see cref="PrivateMessageRequest(string*, long int,
