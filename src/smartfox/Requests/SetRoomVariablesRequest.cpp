...
boost::shared_ptr<std::string>
    SetRoomVariablesRequest::KEY_VAR_ROOM(new std::string("r"));
boost::shared_ptr<std::string>
    SetRoomVariablesRequest::KEY_VAR_LIST(new std::string("vl"));
...
// Init
// -------------------------------------------------------------------
void SetRoomVariablesRequest::Init(
    boost::shared_ptr<std::vector<boost::shared_ptr<RoomVariable>>>
        roomVariables,
    boost::shared_ptr<Room> room) {
...
// Validate
// -------------------------------------------------------------------
void SetRoomVariablesRequest::Validate(const boost::shared_ptr<SmartFox>& sfs) {
    boost::shared_ptr<list<string>> errors(new list<string>());

    // Make sure that the user is joined in the room where variables are going
    // to be set
    if (room != NULL) {
    ...
    if (errors->size() > 0) {
        boost::shared_ptr<std::string> message(
            new std::string("SetRoomVariables request error"));
...
// Execute
// -------------------------------------------------------------------
void SetRoomVariablesRequest::Execute(const boost::shared_ptr<SmartFox>& sfs) {
    boost::shared_ptr<ISFSArray> varList(SFSArray::NewInstance());

    vector<boost::shared_ptr<RoomVariable>>::iterator iterator;
    for (iterator = roomVariables->begin(); iterator != roomVariables->end();
         ++iterator) {
    ...
    sfso->PutInt(KEY_VAR_ROOM, static_cast<std::int32_t>(room->Id()));
}
} // namespace Requests
} // namespace Sfs2X
