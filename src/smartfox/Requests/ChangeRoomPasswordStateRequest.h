...
/// </summary>
/// <remarks>
/// The request not only changes the password of a Room but also its "password
/// state", which indicates if a Room is password protected or not.<br/> This
/// request might fail if the Room was configured to deny password runtime
/// changes or if the User sending the request is not the owner of the Room.
/// <para/>
...
public:
    ...
    /// <example>
    ///        \code{.cpp}
    ///                ptrSmartFox->AddEventListener(SFSEvent::ROOM_PASSWORD_STATE_CHANGE,
    ///boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnRoomPasswordStateChange, (unsigned
    ///long long)this));
    ///                ptrSmartFox->AddEventListener(SFSEvent::ROOM_PASSWORD_STATE_CHANGE_ERROR,
    ///boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnRoomPasswordStateChangeError,
    ...
    ///ChangeRoomPasswordStateRequest(theRoom, "forloop"));
    ///                ptrSmartFox->Send(request);
    ///
    ///                static void OnRoomPasswordStateChange(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);                 static void
    ///OnRoomPasswordStateChangeError(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);
    ///
    ///                void CMyClass::OnRoomPasswordStateChange(unsigned long long
    ...
    ///                }
    ///
    ///                void CMyClass::OnRoomPasswordStateChangeError(unsigned long long
    ///ptrContext, boost::shared_ptr<BaseEvent> ptrEvent);
    ///                {
    ///                    CMyClass* ptrMyClass = (CMyClass*)ptrContext;
    ///
    ...
    ChangeRoomPasswordStateRequest(const boost::shared_ptr<Room>& room,
...
private:
    ...
    boost::shared_ptr<std::string> newPass;
};
} // namespace Requests
} // namespace Sfs2X
