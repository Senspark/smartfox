...
/// </summary>
/// <remarks>
/// This request might fail if the Room was configured not be ranamed or if the
/// User sending the request is not the owner of the Room. <para/> Moderators
/// and Administrator can override this last constraint.
/// </remarks>
...
public:
    ...
    /// <example>
    ///        \code{.cpp}
    ///                ptrSmartFox->AddEventListener(SFSEvent::ROOM_NAME_CHANGE,
    ///boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnRoomNameChange, (unsigned long
    ///long)this));
    ///                ptrSmartFox->AddEventListener(SFSEvent::ROOM_NAME_CHANGE_ERROR,
    ///boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnRoomNameChangeError, (unsigned long
    ///long)this));
    ///
    ...
    ///ChangeRoomNameRequest(theRoom, "Gonzo The Great's Room"));
    ///                ptrSmartFox->Send(request);
    ///
    ///                static void OnRoomNameChange(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);                 static void
    ///OnRoomNameChangeError(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);
    ///
    ///                void CMyClass::OnRoomNameChange(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);
    ///                {
    ...
    ///                    boost::shared_ptr<void> ptrEventParamValueOldName =
    ///(*ptrEventParams)["oldName"];
    ///                    boost::shared_ptr<std::string> ptrOldName =
    ///((boost::static_pointer_cast<string>)(ptrEventParamValueOldName));
    ///                }
    ///
    ///                void CMyClass::OnRoomNameChangeError(unsigned long long ptrContext,
    ///boost::shared_ptr<BaseEvent> ptrEvent);
    ///                {
    ///                    CMyClass* ptrMyClass = (CMyClass*)ptrContext;
    ///
    ...
    ChangeRoomNameRequest(const boost::shared_ptr<Room>& room,
...
private:
    ...
    boost::shared_ptr<std::string> newName;
};
