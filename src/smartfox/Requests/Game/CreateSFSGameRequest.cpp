...
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_IS_PUBLIC(new std::string("gip"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_MIN_PLAYERS(new std::string("gmp"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_INVITED_PLAYERS(new std::string("ginp"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_SEARCHABLE_ROOMS(new std::string("gsr"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_PLAYER_MATCH_EXP(new std::string("gpme"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_SPECTATOR_MATCH_EXP(new std::string("gsme"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_INVITATION_EXPIRY(new std::string("gie"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_LEAVE_ROOM(new std::string("glr"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_NOTIFY_GAME_STARTED(new std::string("gns"));
boost::shared_ptr<std::string>
    CreateSFSGameRequest::KEY_INVITATION_PARAMS(new std::string("ip"));
...
CreateSFSGameRequest::CreateSFSGameRequest(
    ...
    this->createRoomRequest = boost::shared_ptr<CreateRoomRequest>(
        new CreateRoomRequest(settings, false, boost::shared_ptr<Room>()));
}
...
// Validate
// -------------------------------------------------------------------
void CreateSFSGameRequest::Validate(const boost::shared_ptr<SmartFox>& sfs) {
    ...
    if (settings->MinPlayersToStartGame() > settings->MaxUsers())
        errors->push_back(
            "minPlayersToStartGame cannot be greater than maxUsers");

    if (settings->InvitationExpiryTime() <
            InviteUsersRequest::MIN_EXPIRY_TIME ||
        settings->InvitationExpiryTime() >
            InviteUsersRequest::MAX_EXPIRY_TIME) {
        char buffer[512];
        sprintf(buffer, "Expiry time value is out of range (%ld-%ld)",
                InviteUsersRequest::MIN_EXPIRY_TIME,
                InviteUsersRequest::MAX_EXPIRY_TIME);
        string str = buffer;
    ...
    if (counterPlayers > InviteUsersRequest::MAX_INVITATIONS_FROM_CLIENT_SIDE) {
        char buffer[512];
        sprintf(buffer, "Cannot invite more than %ld players from client side",
                InviteUsersRequest::MAX_INVITATIONS_FROM_CLIENT_SIDE);
        string str = buffer;
    ...
    if (errors->size() > 0) {
        boost::shared_ptr<std::string> message(
            new std::string("CreateSFSGame request error"));
...
// Execute
// -------------------------------------------------------------------
void CreateSFSGameRequest::Execute(const boost::shared_ptr<SmartFox>& sfs) {
    ...
    *valueMinPlayersToStartGame =
        (short int)(settings->MinPlayersToStartGame());
    sfso->PutShort(
        KEY_MIN_PLAYERS,
        (boost::static_pointer_cast<short int>)(valueMinPlayersToStartGame));
    ...
    *valueInvitationExpiryTime = (short int)(settings->InvitationExpiryTime());
    sfso->PutShort(
        ...
        (boost::static_pointer_cast<short int>)(valueInvitationExpiryTime));
    ...
    *valueNotifyGameStarted = settings->NotifyGameStarted();
    ...
    if (settings->PlayerMatchExpression() != NULL)
        sfso->PutSFSArray(KEY_PLAYER_MATCH_EXP,
                          settings->PlayerMatchExpression()->ToSFSArray());

    if (settings->SpectatorMatchExpression() != NULL)
        sfso->PutSFSArray(KEY_SPECTATOR_MATCH_EXP,
                          settings->SpectatorMatchExpression()->ToSFSArray());

    // Invited players
    if ((settings->InvitedPlayersUser() != NULL) ||
        (settings->InvitedPlayersBuddy() != NULL)) {
        auto playerIds = boost::make_shared<std::vector<int>>();

        list<boost::shared_ptr<User>>::iterator iteratorUsers;
        list<boost::shared_ptr<Buddy>>::iterator iteratorBuddy;

        for (iteratorUsers = settings->InvitedPlayersUser()->begin();
             iteratorUsers != settings->InvitedPlayersUser()->end();
             iteratorUsers++) {
            boost::shared_ptr<User> player = (*iteratorUsers);
            playerIds->push_back(player->Id());
        }

        for (iteratorBuddy = settings->InvitedPlayersBuddy()->begin();
             iteratorBuddy != settings->InvitedPlayersBuddy()->end();
             iteratorBuddy++) {
    ...
    if (settings->SearchableRooms() != NULL) {
        boost::shared_ptr<std::vector<std::string>> searchableRooms(
            new vector<string>(settings->SearchableRooms()->begin(),
                               settings->SearchableRooms()->end()));
