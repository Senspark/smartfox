...
boost::shared_ptr<std::string>
    FindUsersRequest::KEY_EXPRESSION(new std::string("e"));
boost::shared_ptr<std::string>
    FindUsersRequest::KEY_GROUP(new std::string("g"));
boost::shared_ptr<std::string> FindUsersRequest::KEY_ROOM(new std::string("r"));
boost::shared_ptr<std::string>
    FindUsersRequest::KEY_LIMIT(new std::string("l"));
boost::shared_ptr<std::string>
    FindUsersRequest::KEY_FILTERED_USERS(new std::string("fu"));
...
// -------------------------------------------------------------------
FindUsersRequest::FindUsersRequest(boost::shared_ptr<MatchExpression> expr,
                                   const std::string& target, long int limit)
    : BaseRequest(RequestType_FindUsers) {
    Init(expr, target, limit);
}
...
// Validate
// -------------------------------------------------------------------
void FindUsersRequest::Validate(const boost::shared_ptr<SmartFox>& sfs) {
    ...
    if (errors->size() > 0) {
        boost::shared_ptr<std::string> message(
            new std::string("FindUsers request error"));
...
// Execute
// -------------------------------------------------------------------
void FindUsersRequest::Execute(const boost::shared_ptr<SmartFox>& sfs) {
    sfso->PutSFSArray(KEY_EXPRESSION, matchExpr->ToSFSArray());

    if (targetRoom != NULL) {
        sfso->PutInt(KEY_ROOM, static_cast<std::int32_t>(targetRoom->Id()));
    }
    ...
    if (limit > 0) {
        auto valueLimit = boost::make_shared<std::int16_t>((std::int16_t)limit);
