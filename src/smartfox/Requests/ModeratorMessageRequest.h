...
namespace Requests {
	...
	public:
		...
		///					string* message = new std::string("Message: " +  (*ptrNotifiedMessage));
		///				}
		/// 	\endcode
		/// </example>
		/// <seealso cref="Core.SFSEvent.MODERATOR_MESSAGE"/>
		...
		ModeratorMessageRequest(const std::string& message, boost::shared_ptr<MessageRecipientMode> recipientMode, boost::shared_ptr<ISFSObject> parameters);

		/// <summary>
		/// <see cref="ModeratorMessageRequest(string, boost::shared_ptr<MessageRecipientMode>, boost::shared_ptr<ISFSObject>)"/>
		/// </summary>
		ModeratorMessageRequest(const std::string& message, boost::shared_ptr<MessageRecipientMode> recipientMode);
