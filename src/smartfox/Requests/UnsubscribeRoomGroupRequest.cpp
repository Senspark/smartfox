...
namespace Sfs2X {
namespace Requests {
using Self = UnsubscribeRoomGroupRequest;
...
// Constructor
// -------------------------------------------------------------------
Self::UnsubscribeRoomGroupRequest(const std::string& groupId)
...
// Destructor
// -------------------------------------------------------------------
Self::~UnsubscribeRoomGroupRequest() {}
...
void Self::Validate(const boost::shared_ptr<SmartFox>& sfs) {
    ...
    if (groupId == NULL || groupId->size() == 0)
        errors->push_back(
            "Invalid groupId. Must be a string with at least 1 character.");

    if (errors->size() > 0) {
        boost::shared_ptr<std::string> message(
            new std::string("UnsubscribeGroup request error"));
