...
/// </summary>
/// <remarks>
/// The sender must have the admin privileges to be able to send these kinds of
/// messages.<br/> The <b>recipient</b> parameter can be used to specify if the
/// message is sent to a User, a Room, the Room Group or the whole Zone.
/// </remarks>
///
...
public:
    ...
    /// <example>
    ///            \code{.cpp}
    ///                ptrSmartFox->AddEventListener(SFSEvent::ADMIN_MESSAGE,
    /// boost::shared_ptr<EventListenerDelegate>
    ///(EventListenerDelegate(CMyClass::OnAdminMessage, (unsigned long
    /// long)this));
    ///
    ///             // Say 'Hello' to every one
    ///                boost::shared_ptr<IRequest> request (new
    ///                AdminMessageRequest("Hello
    /// from the Admin!"));                 ptrSmartFox->Send(request);
    ///
    ///                static void OnAdminMessage(unsigned long long ptrContext,
    /// boost::shared_ptr<BaseEvent> ptrEvent);
    ///
    ///                void CMyClass::OnAdminMessage(unsigned long long
    ///                ptrContext,
    /// boost::shared_ptr<BaseEvent> ptrEvent);
    ///                {
    ...
    /// the recipient mode (message can be sent to single User, a Room, the
    /// whole Room Group or the Zone)
    /// </param>
    ...
    /// <summary>
    /// <see cref="AdminMessageRequest(string,
    /// boost::shared_ptr<MessageRecipientMode>,
    ...
    virtual ~AdminMessageRequest() override;
