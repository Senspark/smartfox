...
SFSIOHandler::SFSIOHandler(boost::shared_ptr<BitSwarmClient> bitSwarm) {
    ...
    this->packetEncrypter = boost::shared_ptr<IPacketEncrypter>(
        new DefaultPacketEncrypter(bitSwarm));
...
void SFSIOHandler::Initialize() {
    ...
    protocolCodec = boost::shared_ptr<IProtocolCodec>(
        new SFSProtocolCodec(shared_from_this(), bitSwarm));
}
...
void SFSIOHandler::InitStates() {
    ...
    for (long int enumerator = PACKETREADSTATE_WAIT_NEW_PACKET;
         enumerator <= PACKETREADSTATE_INVALID_DATA; enumerator++) {
        states.push_back(enumerator);
    }
    fsm->AddAllStates(states);

    fsm->AddStateTransition(PACKETREADSTATE_WAIT_NEW_PACKET,
                            PACKETREADSTATE_WAIT_DATA_SIZE,
                            PacketReadTransition_HeaderReceived);
    fsm->AddStateTransition(PACKETREADSTATE_WAIT_DATA_SIZE,
    ...
    fsm->AddStateTransition(PACKETREADSTATE_WAIT_DATA_SIZE_FRAGMENT,
                            PACKETREADSTATE_WAIT_DATA,
                            PacketReadTransition_WholeSizeReceived);
    fsm->AddStateTransition(PACKETREADSTATE_WAIT_DATA,
                            PACKETREADSTATE_WAIT_NEW_PACKET,
                            PacketReadTransition_PacketFinished);

    fsm->AddStateTransition(PACKETREADSTATE_WAIT_DATA,
                            PACKETREADSTATE_INVALID_DATA,
                            PacketReadTransition_InvalidData);
    fsm->AddStateTransition(PACKETREADSTATE_INVALID_DATA,
                            PACKETREADSTATE_WAIT_NEW_PACKET,
...
void SFSIOHandler::OnDataRead(boost::shared_ptr<ByteArray> data) {
    if (data->Length() == 0) {
        boost::shared_ptr<std::string> message(new std::string(
            "Unexpected empty packet data: no readable bytes available!"));
...
SFSIOHandler::HandleNewPacket(boost::shared_ptr<ByteArray> data) {
    boost::shared_ptr<std::vector<std::string>> logMessages(
        new vector<string>());
    auto logMessage =
        StringFormatter("Handling New Packet of size %d", data->Length());
    ...
    if (~(headerByte & 128) > 0) {
        ...
        auto logMessage =
            StringFormatter("Unexpected header byte: %d\n%s", headerByte,
                            DefaultObjectDumpFormatter::HexDump(data)->c_str());
        boost::shared_ptr<SFSError> exception(new SFSError(logMessage));
        throw exception;
    }

    boost::shared_ptr<PacketHeader> header =
        PacketHeader::FromBinary(headerByte);
...
// -------------------------------------------------------------------
boost::shared_ptr<ByteArray>
SFSIOHandler::HandleDataSize(boost::shared_ptr<ByteArray> data) {
    boost::shared_ptr<std::vector<std::string>> logMessages(
        new vector<string>());
    auto logMessage = StringFormatter(
        "Handling Header Size. Length: %d (%s)", data->Length(),
        (pendingPacket->Header()->BigSized() ? "big" : "small"));
    ...
    logMessages->clear();
    logMessage = StringFormatter("Data size is %d", dataSize);
    logMessages->push_back(logMessage);
    log->Debug(logMessages);
...
// -------------------------------------------------------------------
boost::shared_ptr<ByteArray>
SFSIOHandler::HandleDataSizeFragment(boost::shared_ptr<ByteArray> data) {
    boost::shared_ptr<std::vector<string>> logMessages(new vector<string>());
    logMessages->push_back(
        StringFormatter("Handling Size fragment. Data: %d", data->Length()));
    log->Debug(logMessages);

    long int remaining =
        pendingPacket->Header()->BigSized()
            ? (INT_BYTE_SIZE - pendingPacket->Buffer()->Length())
            : (SHORT_BYTE_SIZE - pendingPacket->Buffer()->Length());
    ...
    if (data->Length() >= remaining) {
        ...
        logMessages->clear();
        logMessages->push_back(
            StringFormatter("DataSize is ready: %d bytes", dataSize));
        log->Debug(logMessages);
...
// -------------------------------------------------------------------
boost::shared_ptr<ByteArray>
SFSIOHandler::HandlePacketData(boost::shared_ptr<ByteArray> data) {
    // is there more data for the next incoming packet?
    long int remaining = pendingPacket->Header()->ExpectedLength() -
                         pendingPacket->Buffer()->Length();
    ...
    try {
        boost::shared_ptr<std::vector<string>> logMessages(
            new vector<string>());
        logMessages->push_back(
            StringFormatter("Handling Data: %d, previous state: %d/%d",
                            data->Length(), pendingPacket->Buffer()->Length(),
                            pendingPacket->Header()->ExpectedLength()));
        log->Debug(logMessages);
        ...
        // Not enough data to complete the packet
            ...
            // Add bytes to buffer and let's wait for the rest to come over the
            // network
            pendingPacket->Buffer()->WriteBytes(data->Bytes());
        }
        ...
        boost::shared_ptr<std::vector<string>> logMessages(
            new vector<string>());
        logMessages->push_back(
            StringFormatter("Error handling data: %s", ex.what()));
...
// -------------------------------------------------------------------
boost::shared_ptr<ByteArray>
SFSIOHandler::HandleInvalidData(boost::shared_ptr<ByteArray> data) {
    if (skipBytes == 0) {
    ...
    long int bytesToRemove =
        data->Length() < skipBytes ? data->Length() : skipBytes;
...
// -------------------------------------------------------------------
boost::shared_ptr<ByteArray>
SFSIOHandler::ResizeByteArray(boost::shared_ptr<ByteArray> arrayobj,
                              long int pos, long int len) {
    vector<unsigned char>::iterator iteratorOldArray =
        arrayobj->Bytes()->begin();
    iteratorOldArray += pos;
    boost::shared_ptr<std::vector<unsigned char>> newArray(
        new vector<unsigned char>());
    newArray->insert(newArray->begin(), iteratorOldArray,
                     arrayobj->Bytes()->end());
...
// WriteBinaryData
// -------------------------------------------------------------------
void SFSIOHandler::WriteBinaryData(unsigned long long context,
                                   boost::shared_ptr<PacketHeader> header,
...
void SFSIOHandler::OnDataWrite(boost::shared_ptr<IMessage> message) {
    auto binData = message->Content()->ToBinary();
    ...
    if (binData->Length() > bitSwarm->MaxMessageSize()) {
        ...
        auto err = StringFormatter(
            "Message size is too big: %d, the server limit is: %d",
            binData->Length(), bitSwarm->MaxMessageSize());
    ...
    boost::shared_ptr<PacketHeader> packetHeader(new PacketHeader(
        encrypt, compress, blueBoxed, sizeBytes == INT_BYTE_SIZE));

    if (bitSwarm->Debug()) {
        boost::shared_ptr<std::vector<string>> logMessages(
            new vector<string>());
        logMessages->push_back(StringFormatter(
            "Data written: %s", message->Content()->GetHexDump()->c_str()));
        log->Info(logMessages);
    }

    boost::shared_ptr<WriteBinaryDataDelegate> callback(
        new WriteBinaryDataDelegate(&SFSIOHandler::WriteBinaryData,
                                    (unsigned long long)this));
    boost::shared_ptr<bool> isUDP(new bool());
    *isUDP = message->IsUDP();
    this->bitSwarm->ThreadManager()->EnqueueSend(callback, packetHeader,
                                                 binData, isUDP);
}
