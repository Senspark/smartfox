...
void ThreadManager::ProcessItem(boost::shared_ptr<map<string, boost::shared_ptr<void> > > item)
{
	...
	if (callback != NULL) 
	{
		...
		if (iterator != item->end())
		{
			...
			boost::shared_ptr<std::vector<unsigned char> > data = (boost::static_pointer_cast<std::vector<unsigned char> >)(iterator->second);
			onData->Invoke(data);
...
// EnqueueDataCall
// -------------------------------------------------------------------
void ThreadManager::EnqueueDataCall(boost::shared_ptr<OnDataDelegate> callback, boost::shared_ptr<std::vector<unsigned char> > data)
{
