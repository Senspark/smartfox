...
// SynchRead
// -------------------------------------------------------------------
boost::shared_ptr<std::vector<unsigned char> > TCPClientSSL::SynchRead ()
{
	size_t length = 0;
	boost::system::error_code error;
	...
	if (this->isDisposed == true)
	{
		return boost::shared_ptr<std::vector<unsigned char> >(new vector<unsigned char>());
	}
	...
	{
		return boost::shared_ptr<std::vector<unsigned char> >(new vector<unsigned char>());			// Connection closed cleanly by peer
	}
	else if (error)
	{
...
// AsynchWrite
// -------------------------------------------------------------------
void TCPClientSSL::AsynchWrite (boost::shared_ptr<std::vector<unsigned char> > data, boost::shared_ptr<TCPDataWriteDelegate> callback)
{
	if (this->isDisposed == true)
	{
		return;
...
// SynchWrite
// -------------------------------------------------------------------
void TCPClientSSL::SynchWrite (boost::shared_ptr<std::vector<unsigned char> > data)
{
	if (this->isDisposed == true)
	{
		return;
...
void TCPClientSSL::OnBoostAsioDataReceived(const boost::system::error_code& error, long int length)
{
	...
	if (length > 0)
	{
		data = boost::shared_ptr<std::vector<unsigned char> >(new vector<unsigned char>());
		data->assign((unsigned char*)(boostTcpInputBuffer.data()), (unsigned char*)(boostTcpInputBuffer.data() + length));
	}
	else
	{
