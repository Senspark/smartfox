...
enum SocketErrors {
    ...
    // An invalid pointer address was detected by the underlying socket
    // provider.
    ...
    // on the local machine. This error is returned if the IPv4 address family
    // was specified and the IPv4 stack is not installed on the local machine.
    ...
    // The application tried to send or receive data, and the socket is not
    // connected.
    SocketErrors_NotConnected = 10057,

    // A request to send or receive data was disallowed because the socket has
    // already been closed.
    SocketErrors_Shutdown = 10058,

    // The connection attempt timed out, or the connected host has failed to
    // respond.
