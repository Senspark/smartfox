...
void TCPSocketLayer::InitStates() {
    ...
    for (long int enumerator = TCPSocketLayer::States_Disconnected;
         enumerator <= TCPSocketLayer::States_Connected; enumerator++) {
        states.push_back(enumerator);
    }
    fsm->AddAllStates(states);

    fsm->AddStateTransition(States_Disconnected, States_Connecting,
                            Transitions_StartConnect);
    fsm->AddStateTransition(States_Connecting, States_Connected,
                            Transitions_ConnectionSuccess);
    fsm->AddStateTransition(States_Connecting, States_Disconnected,
                            Transitions_ConnectionFailure);
    fsm->AddStateTransition(States_Connected, States_Disconnected,
...
// LogWarn
// -------------------------------------------------------------------
void TCPSocketLayer::LogWarn(const boost::shared_ptr<std::string>& msg) {
    lockDispose.lock();
...
// LogError
// -------------------------------------------------------------------
void TCPSocketLayer::LogError(const boost::shared_ptr<std::string>& msg) {
    lockDispose.lock();
...
// HandleError
// -------------------------------------------------------------------
void TCPSocketLayer::HandleError(const boost::shared_ptr<std::string>& err) {
...
void TCPSocketLayer::HandleError(const boost::shared_ptr<std::string>& err,
    ...
    boost::shared_ptr<std::map<string, boost::shared_ptr<void>>> data(
        new std::map<string, boost::shared_ptr<void>>());
    ...
    boost::shared_ptr<std::string> convertedString(
        new std::string(converterIntToString.str()));
    data->insert(
        std::pair<string, boost::shared_ptr<void>>("se", convertedString));

    boost::shared_ptr<ParameterizedThreadStart> callback(
        new ParameterizedThreadStart(&TCPSocketLayer::HandleErrorCallback,
...
// HandleErrorCallback
// -------------------------------------------------------------------
void TCPSocketLayer::HandleErrorCallback(unsigned long long context,
                                         boost::shared_ptr<void> state) {
    // Map context
    auto instance = reinterpret_cast<TCPSocketLayer*>(context);

    auto data = boost::static_pointer_cast<
        std::map<std::string, boost::shared_ptr<void>>>(state);

    auto err = boost::static_pointer_cast<std::string>(data->at("err"));

    int errAsInteger = 0;
    *boost::static_pointer_cast<std::string>(data->at("se")) =
        StringFormatter("%d", errAsInteger);
...
// CallOnError
// -------------------------------------------------------------------
void TCPSocketLayer::CallOnError(const boost::shared_ptr<std::string>& msg,
                                 SocketErrors se) {
    if (onError != NULL) {
...
// CallOnData
// -------------------------------------------------------------------
void TCPSocketLayer::CallOnData(
...
// OnSocketConnection
// -------------------------------------------------------------------
void TCPSocketLayer::OnSocketConnection(unsigned long long context,
                                        SocketErrors code) {
    // Map context
    ...
    if (code == SocketErrors_Success) {
        ...
        boost::shared_ptr<TCPDataReadDelegate> callback(new TCPDataReadDelegate(
            &TCPSocketLayer::OnSocketDataReceived, context));
        instance->connection->AsynchRead(callback);
    } else {
        boost::shared_ptr<std::string> message(
            new std::string("Connection error: "));
...
// OnSocketDataSent
// -------------------------------------------------------------------
void TCPSocketLayer::OnSocketDataSent(unsigned long long context,
                                      SocketErrors code) {
    // Map context
    TCPSocketLayer* instance = (TCPSocketLayer*)context;
    ...
    if (code != SocketErrors_Success) {
        boost::shared_ptr<std::string> message(
            new std::string("Error sending data to remote side over the "
...
// -------------------------------------------------------------------
void TCPSocketLayer::OnSocketDataReceived(
    ...
    if (data->size() < 1) {
        boost::shared_ptr<std::string> message(
            new std::string("Connection closed by the remote side"));
...
// HandleBinaryData
// -------------------------------------------------------------------
void TCPSocketLayer::HandleBinaryData(
    boost::shared_ptr<std::vector<unsigned char>> data) {
    CallOnData(data);
}
...
// WriteSocket
// -------------------------------------------------------------------
void TCPSocketLayer::WriteSocket(
    boost::shared_ptr<std::vector<unsigned char>> data) {
    if (State() != States_Connected) {
        boost::shared_ptr<std::string> message(
            new std::string("Trying to write to disconnected socket"));
        LogError(message);
    ...
    try {
        boost::shared_ptr<TCPDataWriteDelegate> callback(
            new TCPDataWriteDelegate(&TCPSocketLayer::OnSocketDataSent,
                                     (unsigned long long)this));
        ...
        boost::shared_ptr<std::string> message(
            new std::string("General error writing to socket: "));
        HandleError(message);
    }
}
...
// Connect
// -------------------------------------------------------------------
void TCPSocketLayer::Connect(boost::shared_ptr<IPAddress> address,
                             unsigned short int port) {
    if (State() != States_Disconnected) {
        boost::shared_ptr<std::string> message(new std::string(
            "Calling connect when the socket is not disconnected"));
        LogWarn(message);
        return;
    }
    ...
    try {
        // Below we create a TCPClient instance that will be used as connection
        // to exchange data with remote SmartFoxServer At API startup we have
        // not a connection entity up to the below statement will be executed
        // but when an API reconnection occurs we already have a "pending"
        // connection To avoid that asynch callbacks (data received or data
        // written callbacks) will be called on instance disposed by smart
        // pointers we must sure that no callbacks are in progress and we mark
        // the instance as disposed before create a new TCPClient
        if (connection != NULL) {
            connection->Dispose();
        ...
        connection = boost::shared_ptr<TCPClient>(
            new TCPClient(*(bitSwarm->GetBoostService())));

        this->me = shared_from_this();
        boost::shared_ptr<TCPConnectionDelegate> callback(
            new TCPConnectionDelegate(&TCPSocketLayer::OnSocketConnection,
                                      (unsigned long long)this));
        connection->Connect(ipAddress, socketNumber, Const_TimeoutConnect,
                            callback);
        ...
        boost::thread t(
            boost::bind(static_cast<size_t (boost::asio::io_service::*)()>(
                        ...
                        bitSwarm->GetBoostService()));
    } catch (...) {
        boost::shared_ptr<std::string> message(
            new std::string("General exception on connection: "));
        HandleError(message);
    }
}
...
void TCPSocketLayer::Disconnect() {
    if (State() != States_Connected) {
        boost::shared_ptr<std::string> message(new std::string(
            "Calling disconnect when the socket is not connected"));
        LogWarn(message);
        return;
    }
...
// OnConnect
// -------------------------------------------------------------------
void TCPSocketLayer::OnConnect(
    boost::shared_ptr<ConnectionDelegate> onConnect) {
...
// OnDisconnect
// -------------------------------------------------------------------
void TCPSocketLayer::OnDisconnect(
    boost::shared_ptr<ConnectionDelegate> onDisconnect) {
...
// Write
// -------------------------------------------------------------------
void TCPSocketLayer::Write(boost::shared_ptr<std::vector<unsigned char>> data) {
    WriteSocket(data);
}
