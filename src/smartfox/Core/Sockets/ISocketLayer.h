...
typedef DelegateOneArgument<boost::shared_ptr<std::vector<unsigned char>>>
    OnDataDelegate;
typedef DelegateTwoArguments<boost::shared_ptr<std::string>,
                             Sfs2X::Core::Sockets::SocketErrors>
    OnErrorDelegate;
...
class ISocketLayer {
public:
    virtual ~ISocketLayer() = default;
    virtual void Connect(boost::shared_ptr<IPAddress> address,
                         unsigned short int port) = 0;
    virtual bool IsConnected() = 0;
    virtual bool RequiresConnection() = 0;
    virtual void Disconnect() = 0;

    virtual boost::shared_ptr<ConnectionDelegate>
    OnConnect() = 0; // Get delegate
    virtual void OnConnect(
        boost::shared_ptr<ConnectionDelegate> onConnect) = 0; // Set delegate

    virtual boost::shared_ptr<ConnectionDelegate>
    OnDisconnect() = 0; // Get delegate
    virtual void OnDisconnect(
        boost::shared_ptr<ConnectionDelegate> onDisconnect) = 0; // Set delegate

    virtual void Write(boost::shared_ptr<std::vector<unsigned char>>) = 0;

    virtual boost::shared_ptr<OnDataDelegate> OnData() = 0; // Get delegate
    virtual void
    OnData(boost::shared_ptr<OnDataDelegate> onData) = 0; // Set delegate

    virtual boost::shared_ptr<OnErrorDelegate> OnError() = 0; // Get delegate
    virtual void
    OnError(boost::shared_ptr<OnErrorDelegate> onError) = 0; // Set delegate
