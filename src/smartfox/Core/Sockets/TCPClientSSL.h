...
typedef DelegateOneArgument<boost::shared_ptr<std::vector<unsigned char>>>
    TCPDataReadDelegate;
...
public:
    ...
    //        context            SSL context
    TCPClientSSL(boost::asio::io_service& io_service,
    ...
    void Connect(boost::shared_ptr<IPAddress> ipAddress,
                 unsigned short int ipPort, long int timeout,
    ...
    //        ipAddress        Server ip address expressed in dotted form
    //        ipPort            Server ip port
    void SynchConnect(boost::shared_ptr<IPAddress> ipAddress,
    ...
    void AsynchWrite(boost::shared_ptr<std::vector<unsigned char>> data,
    ...
    boost::shared_ptr<std::vector<unsigned char>> SynchRead();

    // SynchWrite
    // Synchronous write data on the underlying socket
    //        data        Buffer that contains data to write
    void SynchWrite(boost::shared_ptr<std::vector<unsigned char>> data);
...
private:
    ...
    bool OnBoostAsioVerifyCertificate(bool preverified,
    ...
    // Callback to receive Boost Asio notification regarding timedout connect
    // task
    //        code        Error code
    ...
    void OnBoostAsioDataReceived(const boost::system::error_code& error,
                                 long int length);
    ...
    //        error                Error code
    void OnBoostAsioDataSent(boost::shared_ptr<unsigned char> bufferOutput,
    ...
    boost::asio::ssl::stream<tcp::socket>
        boostTcpSocket; // Boost Asio Tcp socket
    boost::asio::deadline_timer
        boostTimerTaskDeadline; // Boost Asio deadline timer
    boost::array<char, READ_BUFFER_SIZE>
        boostTcpInputBuffer; // Boost Asio Tcp input buffer

    boost::shared_ptr<TCPConnectionDelegate>
        callbackTCPConnection; // Callback to notify tcp connection result
    boost::shared_ptr<TCPDataReadDelegate>
        callbackTCPDataRead; // Callback to notify tcp received data
    boost::shared_ptr<TCPDataWriteDelegate> callbackTCPDataWrite; // Callback to
                                                                  // notify the
                                                                  // completion
                                                                  // of tcp
                                                                  // write data
    ...
    boost::recursive_mutex
        mtxDisconnection; // Mutex to synchronize Tcp disconnection tasks

    long int counterAsyncReadOperationsInProgress;  // Counter of asynchronous
                                                    // read operations that are
                                                    // in progress
    long int counterAsyncWriteOperationsInProgress; // Counter of asynchronous
