...
public:
    virtual ~IBuddyManager() = default;

    /// <summary>
    /// Checks if the current User's Buddy List is inited.
    /// </summary>
    /// <remarks>
    /// If not you should send an InitBuddyListRequest to the server in order to
    /// retrieve your persistent Buddy List data.
    /// </remarks>
    ...
    virtual void AddBuddy(const boost::shared_ptr<Buddy>& buddy) = 0;

    virtual boost::shared_ptr<Buddy> RemoveBuddyById(int id) = 0;

    virtual boost::shared_ptr<Buddy>
    RemoveBuddyByName(const std::string& name) = 0;
    ...
    virtual bool ContainsBuddy(const std::string& name) = 0;
    ...
    /// </returns>
    /// <seealso cref="SFSBuddy"/>
    virtual boost::shared_ptr<Buddy> GetBuddyById(int id) = 0;
    ...
    /// </returns>
    virtual boost::shared_ptr<Buddy>
    GetBuddyByName(const std::string& name) = 0;
    ...
    /// <seealso cref="SFSBuddy"/>
    virtual boost::shared_ptr<Buddy>
    GetBuddyByNickName(const std::string& nickName) = 0;
    ...
    /// <summary>
    /// Get the User's buddy list. It could be null if the Buddy List was not
    /// initialized
    /// </summary>
    /// <seealso cref="Requests.InitBuddyListRequest"/>
    virtual boost::shared_ptr<std::vector<boost::shared_ptr<Buddy>>>
    ...
    /// <summary>
    /// Get a list of Strings representing the custom Buddy states that the
    /// application can use. The custom states are sent upon initialization of
    /// the Buddy List and are configured on the server side.
    /// </summary>
    virtual boost::shared_ptr<std::vector<std::string>> BuddyStates() = 0;
    ...
    /// </summary>
    virtual void
    BuddyStates(const boost::shared_ptr<std::vector<std::string>>& value) = 0;
    ...
    GetMyVariable(const boost::shared_ptr<std::string>& varName) = 0;
    ...
    /// </returns>
    virtual boost::shared_ptr<BuddyVariable>
    GetMyVariable(const std::string& varName) = 0;

    /// <summary>
    /// Get all current User's Buddy Variables
    /// </summary>
    /// <remarks>
    /// This operation is valid only if the User's BuddyList has already been
    /// initialized
    /// </remarks>
    ...
    /// <remarks>
    /// This operation is valid only if the User's BuddyList has already been
    /// initialized<br/> The value is handled by a reserved Buddy Variable
    /// <para/>
    ...
    /// </remarks>
    /// <seealso cref="ReservedBuddyVariables"/>
    virtual boost::shared_ptr<std::string> MyNickName() = 0;
    ...
    /// </remarks>
    /// <seealso cref="ReservedBuddyVariables"/>
    virtual void MyNickName(const std::string& value) = 0;

    /// <summary>
    /// Get the current User's optional custom state (e.g. "Available"; "Busy",
    /// "Be right back"...)
    /// </summary>
    /// <remarks>
    /// This operation is valid only if the User's BuddyList has already been
    ...
    /// </remarks>
    /// <seealso cref="ReservedBuddyVariables"/>
    virtual boost::shared_ptr<std::string> MyState() = 0;
    ...
    /// </remarks>
    /// <seealso cref="ReservedBuddyVariables"/>
    virtual void MyState(const std::string& value) = 0;

    virtual void
    SetMyVariable(const boost::shared_ptr<BuddyVariable>& bVar) = 0;
