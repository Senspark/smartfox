...
// Constructor
// -------------------------------------------------------------------
SFSGlobalUserManager::SFSGlobalUserManager(const boost::shared_ptr<Room>& room)
	: SFSUserManager(room)
{
	roomRefCount.clear();
}
...
// AddUser
// -------------------------------------------------------------------
void SFSGlobalUserManager::AddUser(const boost::shared_ptr<User>& user)
{
...
// RemoveUser
// -------------------------------------------------------------------
void SFSGlobalUserManager::RemoveUser(const boost::shared_ptr<User>& user)
{
	lockRoomRefCount.lock();
	...
	if (iterator != roomRefCount.end())
	{
		...
		{
			boost::shared_ptr<std::string> message (new std::string("GlobalUserManager RefCount is already at zero. User: " + (*user->Name())));
			LogWarn(message);
	...
	{
		boost::shared_ptr<std::string> message(new std::string("Can't remove User from GlobalUserManager. RefCount missing. User: " + (*user->Name())));
		LogWarn(message);
	}
