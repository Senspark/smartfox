...
SFSBuddy::FromSFSArray(const boost::shared_ptr<ISFSArray>& arr) {
    boost::shared_ptr<Buddy> buddy(new SFSBuddy(
        arr->GetInt(0),          // id
        *(arr->GetUtfString(1)), // name
        arr->GetBool(2),         // blocked
        arr->Size() > 4 ? arr->GetBool(4)
                        : false // isTemp is optional, we have to check
        ));

    boost::shared_ptr<ISFSArray> bVarsData =
        arr->GetSFSArray(3); // variables data array

    for (std::size_t j = 0; j < bVarsData->Size(); j++) {
        boost::shared_ptr<BuddyVariable> bv =
            SFSBuddyVariable::FromSFSArray(bVarsData->GetSFSArray(j));
...
// SFSBuddy
// -------------------------------------------------------------------
SFSBuddy::SFSBuddy(int id, const std::string& name) {
    this->id = id;
...
// SFSBuddy
// -------------------------------------------------------------------
SFSBuddy::SFSBuddy(int id, const std::string& name, bool isBlocked) {
    this->id = id;
    this->name = boost::shared_ptr<std::string>(new std::string(name));
...
// Name
// -------------------------------------------------------------------
boost::shared_ptr<std::string> SFSBuddy::Name() const {
    return name;
}
...
// FromSFSArray
// -------------------------------------------------------------------
bool SFSBuddy::IsOnline() const {
    boost::shared_ptr<BuddyVariable> bv =
        GetVariable(*ReservedBuddyVariables::BV_ONLINE);
...
// State
// -------------------------------------------------------------------
boost::shared_ptr<std::string> SFSBuddy::State() const {
    /*
     * TODO: what if state was not inited yet?
     * Do we return null or a default state?
     */
    boost::shared_ptr<BuddyVariable> bv =
        GetVariable(*ReservedBuddyVariables::BV_STATE);
    return (bv == NULL) ? boost::shared_ptr<std::string>()
                        : bv->GetStringValue();
}
...
// NickName
// -------------------------------------------------------------------
boost::shared_ptr<std::string> SFSBuddy::NickName() const {
    boost::shared_ptr<BuddyVariable> bv =
        GetVariable(*ReservedBuddyVariables::BV_NICKNAME);
    return (bv == NULL) ? boost::shared_ptr<std::string>()
                        : bv->GetStringValue();
}
...
SFSBuddy::Variables() const {
    ...
    for (iterator = this->variables->begin();
         iterator != this->variables->end(); iterator++) {
...
SFSBuddy::GetVariable(const std::string& varName) const {
    std::map<string, boost::shared_ptr<BuddyVariable>>::iterator iterator =
...
// -------------------------------------------------------------------
boost::shared_ptr<std::vector<boost::shared_ptr<BuddyVariable>>>
SFSBuddy::GetOfflineVariables() const {
    boost::shared_ptr<std::vector<boost::shared_ptr<BuddyVariable>>>
        offlineVars(new vector<boost::shared_ptr<BuddyVariable>>());
...
// -------------------------------------------------------------------
boost::shared_ptr<std::vector<boost::shared_ptr<BuddyVariable>>>
SFSBuddy::GetOnlineVariables() const {
    boost::shared_ptr<std::vector<boost::shared_ptr<BuddyVariable>>> onlineVars(
        new vector<boost::shared_ptr<BuddyVariable>>());
...
// ContainsVariable
// -------------------------------------------------------------------
bool SFSBuddy::ContainsVariable(const std::string& varName) const {
    std::map<string, boost::shared_ptr<BuddyVariable>>::iterator iterator =
        variables->find(varName);
...
void SFSBuddy::SetVariable(boost::shared_ptr<BuddyVariable> bVar) {
    std::map<string, boost::shared_ptr<BuddyVariable>>::iterator iterator =
        variables->find(*bVar->Name());

    if (iterator != variables->end()) {
        iterator->second = bVar;
    } else {
        variables->insert(pair<string, boost::shared_ptr<BuddyVariable>>(
            *bVar->Name(), bVar));
    }
}
...
// RemoveVariable
// -------------------------------------------------------------------
void SFSBuddy::RemoveVariable(const std::string& varName) {
    std::map<string, boost::shared_ptr<BuddyVariable>>::iterator iterator;
...
void SFSBuddy::ClearVolatileVariables() {
    ...
    for (iteratorNamesToRemove = listNamesToRemove.begin();
         iteratorNamesToRemove != listNamesToRemove.end();
         iteratorNamesToRemove++) {
...
// ToString
// -------------------------------------------------------------------
boost::shared_ptr<std::string> SFSBuddy::ToString() const {
    char buffer[512];
