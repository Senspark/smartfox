...
namespace Entities {
	...
	public:
		...
		/// <exclude/>
		MMORoom (long int id, const std::string& name, const std::string& groupId);
		
		/// <exclude/>
		MMORoom (long int id, const std::string& name);
		...
		boost::shared_ptr<std::vector<boost::shared_ptr<IMMOItem> > > GetMMOItems();
