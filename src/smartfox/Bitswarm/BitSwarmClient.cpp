...
BitSwarmClient::BitSwarmClient() {
    ...
    lastIpAddress = boost::shared_ptr<std::string>(new std::string());
    lastTcpPort = 0;
    ...
    bbConnected = false;
    isDisposed = false;
    threadManager = boost::shared_ptr<Sfs2X::Core::ThreadManager>(
        new Sfs2X::Core::ThreadManager());
    ioHandler = boost::shared_ptr<Sfs2X::Bitswarm::IoHandler>();
    manualDisconnection = false;
    //    retryTimer = boost::shared_ptr<boost::asio::deadline_timer>(new
    //    boost::asio::deadline_timer(*(this->GetBoostService())));
    io_service_retryTimer = boost::shared_ptr<boost::asio::io_service>(
        new boost::asio::io_service());
    retryTimer = boost::shared_ptr<boost::asio::deadline_timer>(
        new boost::asio::deadline_timer(*io_service_retryTimer));
}
...
BitSwarmClient::BitSwarmClient(boost::shared_ptr<SmartFox> sfs) {
    ...
    bbConnected = false;
    isDisposed = false;
    connectionMode = boost::shared_ptr<std::string>();
    threadManager = boost::shared_ptr<Sfs2X::Core::ThreadManager>(
        new Sfs2X::Core::ThreadManager());
    ioHandler = boost::shared_ptr<Sfs2X::Bitswarm::IoHandler>();
    manualDisconnection = false;
...
void BitSwarmClient::Dispose() {
    ...
    if (socket != NULL) {
        ...
        boost::shared_ptr<ConnectionDelegate> delegateConnection =
            boost::shared_ptr<ConnectionDelegate>();
        socket->OnConnect(delegateConnection);

        boost::shared_ptr<ConnectionDelegate> delegateDisconnection =
            boost::shared_ptr<ConnectionDelegate>();
        socket->OnDisconnect(delegateDisconnection);

        boost::shared_ptr<OnDataDelegate> delegateOnData =
            boost::shared_ptr<OnDataDelegate>();
        socket->OnData(delegateOnData);

        boost::shared_ptr<OnErrorDelegate> delegateOnError =
            boost::shared_ptr<OnErrorDelegate>();
    ...
    for (iterator = controllers.begin(); iterator != controllers.end();
         iterator++) {
...
// ConnectionMode
// -------------------------------------------------------------------
boost::shared_ptr<std::string> BitSwarmClient::ConnectionMode() {
...
// IoHandler
// -------------------------------------------------------------------
void BitSwarmClient::IoHandler(
    boost::shared_ptr<Sfs2X::Bitswarm::IoHandler> value) {
    if (value == NULL) {
        boost::shared_ptr<std::string> message(
            new std::string("IOHandler is already set!"));
...
void BitSwarmClient::CompressionThreshold(long int value) {
    ...
    if (value > 100) {
        ...
        boost::throw_exception(boost::enable_error_info(std::runtime_error(
            "Compression threshold cannot be < 100 bytes.")));
...
boost::shared_ptr<Logger> BitSwarmClient::Log() {
    if (sfs == NULL) {
        ...
        return boost::shared_ptr<Logger>(
            new Logger(boost::shared_ptr<SmartFox>()));
    }
...
// CryptoKey
// -------------------------------------------------------------------
void BitSwarmClient::CryptoKey(
    boost::shared_ptr<Sfs2X::Util::CryptoKey> value) {
...
void BitSwarmClient::ForceBlueBox(bool val) {
    if (!bbConnected) {
        ...
        boost::throw_exception(boost::enable_error_info(
            std::runtime_error("You can't change the BlueBox mode while the "
                               "connection is running")));
        // throw exception("You can't change the BlueBox mode while the
        // connection is running");
    }
}
...
void BitSwarmClient::Init() {
    ...
    if (socket != NULL)
        return; // Don't init the socket layer if it's already initialized

    socket =
        boost::shared_ptr<ISocketLayer>(new TCPSocketLayer(shared_from_this()));

    boost::shared_ptr<ConnectionDelegate> delegateConnection(
        new ConnectionDelegate(&BitSwarmClient::OnSocketConnect,
                               (unsigned long long)this));
    socket->OnConnect(delegateConnection);

    boost::shared_ptr<ConnectionDelegate> delegateDisconnection(
        new ConnectionDelegate(&BitSwarmClient::OnSocketClose,
                               (unsigned long long)this));
    socket->OnDisconnect(delegateDisconnection);

    boost::shared_ptr<OnDataDelegate> delegateOnData(new OnDataDelegate(
        &BitSwarmClient::OnSocketData, (unsigned long long)this));
    socket->OnData(delegateOnData);

    boost::shared_ptr<OnErrorDelegate> delegateOnError(new OnErrorDelegate(
        &BitSwarmClient::OnSocketError, (unsigned long long)this));
    socket->OnError(delegateOnError);

    bbClient = boost::shared_ptr<BBClient>(new BBClient(shared_from_this()));
    bbClient->Initialize();

    bbClient->AddEventListener(
        BBEvent::CONNECT,
        boost::shared_ptr<EventListenerDelegate>(new EventListenerDelegate(
            &BitSwarmClient::OnBBConnect, (unsigned long long)this)));
    bbClient->AddEventListener(
        BBEvent::DATA,
        boost::shared_ptr<EventListenerDelegate>(new EventListenerDelegate(
            &BitSwarmClient::OnBBData, (unsigned long long)this)));
    bbClient->AddEventListener(
        BBEvent::DISCONNECT,
        boost::shared_ptr<EventListenerDelegate>(new EventListenerDelegate(
            &BitSwarmClient::OnBBDisconnect, (unsigned long long)this)));
    bbClient->AddEventListener(
        BBEvent::IO_ERROR,
        boost::shared_ptr<EventListenerDelegate>(new EventListenerDelegate(
            &BitSwarmClient::OnBBError, (unsigned long long)this)));
    bbClient->AddEventListener(
        BBEvent::SECURITY_ERROR,
        boost::shared_ptr<EventListenerDelegate>(new EventListenerDelegate(
            &BitSwarmClient::OnBBError, (unsigned long long)this)));
...
// ConnectionIp
// -------------------------------------------------------------------
boost::shared_ptr<std::string> BitSwarmClient::ConnectionIp() {
    if (!Connected()) {
        return boost::shared_ptr<std::string>(new std::string("Not Connected"));
    } else {
        return boost::shared_ptr<std::string>(new std::string(*lastIpAddress));
    }
}
...
// AddController
// -------------------------------------------------------------------
void BitSwarmClient::AddController(long int id,
                                   boost::shared_ptr<IController> controller) {
    if (controller == NULL) {
        boost::throw_exception(boost::enable_error_info(
            std::runtime_error("Controller is null, it can't be added.")));
    ...
    if (iterator != controllers.end()) {
        auto err = StringFormatter("A controller with id:  %d already exists! "
                                   "Controller can't be added:",
                                   id);

        boost::throw_exception(
            boost::enable_error_info(std::runtime_error(err.c_str())));
...
{
    IController controller = Activator.CreateInstance(controllerType) as
IController; AddController(id, controller);
}
*/
// -------------------------------------------------------------------
// Connect
...
void BitSwarmClient::Connect() {
    boost::shared_ptr<std::string> address(new std::string("127.0.0.1"));
...
// Connect
// -------------------------------------------------------------------
void BitSwarmClient::Connect(const boost::shared_ptr<std::string>& ip,
    ...
    if (useBlueBox) {
        ...
        bbClient->PollSpeed((sfs->Config() != NULL)
                                ? sfs->Config()->BlueBoxPollingRate()
                                : 750);
...
void BitSwarmClient::Disconnect() {
    Disconnect(boost::shared_ptr<std::string>());
}

// -------------------------------------------------------------------
// Disconnect
// -------------------------------------------------------------------
void BitSwarmClient::Disconnect(const boost::shared_ptr<std::string>& reason) {
...
void BitSwarmClient::InitControllers() {
    auto sysController =
        boost::make_shared<Sfs2X::Controllers::SystemController>(
            shared_from_this());
    sysController->Initialize();

    auto extController =
        boost::make_shared<Sfs2X::Controllers::ExtensionController>(
            shared_from_this());
...
void BitSwarmClient::OnSocketConnect(unsigned long long context) {
    auto instance = reinterpret_cast<BitSwarmClient*>(context);
    ...
    auto evt = boost::make_shared<BitSwarmEvent>(BitSwarmEvent::CONNECT);
    auto args =
        boost::make_shared<std::map<std::string, boost::shared_ptr<void>>>();
    args->emplace("success", boost::make_shared<bool>(true));
    // 2nd argument not publicly documented, used internally
    args->emplace("isReconnection",
                  boost::make_shared<bool>(instance->attemptingReconnection));
    evt->Params(args);
...
void BitSwarmClient::OnSocketClose(unsigned long long context) {
    ...
    bool isRegularDisconnection =
        instance->sfs == NULL || (!instance->attemptingReconnection &&
                                  instance->sfs->GetReconnectionSeconds() == 0);
    // bool isManualDisconnection = (evt!=null) && (string)evt.Params["reason"]
     ...
     *        In this case a disconnection means that there's no way we can
     * reconnect, so end of story.
     *
     *    2.    It is Regular Disconnection (no reconnection feature available)
     *
     *    3.    It is a voluntary (Manual) disconnection, initiated on the
     * client
     *
     */
    if (instance->attemptingReconnection || isRegularDisconnection ||
         ...
         * Dispatching the event here only in the case of manual disconnection.
         * If it's disconnection from server This will be dispatched from the
         * server disconnection event handler
         */

        if (isManualDisconnection) {
            boost::shared_ptr<map<string, boost::shared_ptr<void>>> args(
                new map<string, boost::shared_ptr<void>>());
            args->emplace("reason", ClientDisconnectionReason::MANUAL);

            boost::shared_ptr<BitSwarmEvent> evt(
                new BitSwarmEvent(BitSwarmEvent::DISCONNECT, args));
            instance->sfs->DispatchEvent(evt);

            // Don't resetting manualDisconnection flag here as there will be
            // OnSocketError event
        }
    ...
    char buffer[512];
    sprintf(buffer, "Attempting reconnection in %ld sec",
            instance->ReconnectionSeconds());
    ...
    if (instance->attemptingReconnection == false) {
        /*
         * If we aren't in any of the above three cases then it's time to
         * attempt a reconnection to the server.
         */
        ...
        boost::shared_ptr<BitSwarmEvent> evt(
            new BitSwarmEvent(BitSwarmEvent::RECONNECTION_TRY));
        instance->DispatchEvent(evt);
...
void BitSwarmClient::ExecuteDisconnection() {
    boost::shared_ptr<map<string, boost::shared_ptr<void>>> args(
        new map<string, boost::shared_ptr<void>>());
    args->emplace("reason", ClientDisconnectionReason::UNKNOWN);
     ...
     * --==|| Clear resources ||==--
     *
     * Without this all thread will remain active even if the API are done with
     * the Session
     *
     * @since 1.0.8
     * @by Lapo
...
void BitSwarmClient::RetryConnection(long int timeout) {
    ...
    //    retryTimer->async_wait(boost::bind(&BitSwarmClient::OnRetryConnectionEvent,
...
// -------------------------------------------------------------------
void BitSwarmClient::OnRetryConnectionEvent(
...
// OnSocketData
// -------------------------------------------------------------------
void BitSwarmClient::OnSocketData(
    ...
    try {
        ...
        boost::shared_ptr<std::string> message(new std::string(e.what()));
        ...
        boost::shared_ptr<BitSwarmEvent> evt(
            new BitSwarmEvent(BitSwarmEvent::DATA_ERROR));
        boost::shared_ptr<map<string, boost::shared_ptr<void>>> args(
            new map<string, boost::shared_ptr<void>>());
...
// OnSocketError
// -------------------------------------------------------------------
void BitSwarmClient::OnSocketError(unsigned long long context,
                                   boost::shared_ptr<std::string> message,
                                   SocketErrors se) {
    BitSwarmClient* instance = (BitSwarmClient*)context;
    ...
    boost::shared_ptr<std::string> messageFormatted(new std::string(buffer));

    data->emplace("reason", messageFormatted);

    if (se != SocketErrors_AccessDenied) {
        if (!instance->attemptingReconnection &&
            !instance->sfs->IsConnecting() && !instance->manualDisconnection) {
            boost::shared_ptr<map<string, boost::shared_ptr<void>>>
            disconnectData(new map<string, boost::shared_ptr<void>>());
            disconnectData->insert(pair<string, boost::shared_ptr<void>>(
                "reason", (boost::shared_ptr<std::string>)(new std::string(
                              *ClientDisconnectionReason::UNKNOWN))));
            evt = boost::shared_ptr<BitSwarmEvent>(
                new BitSwarmEvent(BitSwarmEvent::DISCONNECT));
            evt->Params(disconnectData);
            instance->DispatchEvent(evt);
        }

        evt = boost::shared_ptr<BitSwarmEvent>(
            new BitSwarmEvent(BitSwarmEvent::IO_ERROR));
        evt->Params(data);
    } else {
        evt = boost::shared_ptr<BitSwarmEvent>(
            new BitSwarmEvent(BitSwarmEvent::SECURITY_ERROR));
        evt->Params(data);
    }
...
// AddEventListener
// -------------------------------------------------------------------
void BitSwarmClient::AddEventListener(
...
void BitSwarmClient::DispatchEvent(boost::shared_ptr<BitSwarmEvent> evt) {
    ...
    if (evt->Params() != NULL) {
        ...
        for (itrArguments = evt->Params()->begin();
             itrArguments != evt->Params()->end(); itrArguments++) {
            if ((itrArguments->first == "success") ||
                (itrArguments->first == "message") ||
                (itrArguments->first == "reason")) {
                continue;
            }
...
// OnBBConnect
// -------------------------------------------------------------------
void BitSwarmClient::OnBBConnect(unsigned long long context,
    ...
    boost::shared_ptr<BitSwarmEvent> evt(
        new BitSwarmEvent(BitSwarmEvent::CONNECT));
    boost::shared_ptr<map<string, boost::shared_ptr<void>>> args(
        new map<string, boost::shared_ptr<void>>());
    ...
    *valueSuccess = true;
    args->insert(
        pair<string, boost::shared_ptr<void>>("success", valueSuccess));
    boost::shared_ptr<bool> valueIsReconnection(new bool());
    *valueIsReconnection = false;
    args->insert(pair<string, boost::shared_ptr<void>>("isReconnection",
                                                       valueIsReconnection));
    evt->Params(args);
    instance->DispatchEvent(evt);
}
...
// OnBBData
// -------------------------------------------------------------------
void BitSwarmClient::OnBBData(unsigned long long context,
                              boost::shared_ptr<BaseEvent> e) {
    BitSwarmClient* instance = (BitSwarmClient*)context;

    boost::shared_ptr<BBEvent> evt = (boost::static_pointer_cast<BBEvent>)(e);
    boost::shared_ptr<ByteArray> buffer =
        (boost::static_pointer_cast<ByteArray>)((*(evt->Params()))["data"]);
...
// OnBBDisconnect
// -------------------------------------------------------------------
void BitSwarmClient::OnBBDisconnect(unsigned long long context,
                                    boost::shared_ptr<BaseEvent> e) {
    BitSwarmClient* instance = (BitSwarmClient*)context;
...
// OnBBError
// -------------------------------------------------------------------
void BitSwarmClient::OnBBError(unsigned long long context,
                               boost::shared_ptr<BaseEvent> e) {
    BitSwarmClient* instance = (BitSwarmClient*)context;

    boost::shared_ptr<BBEvent> evt = (boost::static_pointer_cast<BBEvent>)(e);

    boost::shared_ptr<std::string> evtMessage =
        (boost::static_pointer_cast<string>)((*(evt->Params()))["message"]);
    ...
    boost::shared_ptr<BitSwarmEvent> bevt(
        new BitSwarmEvent(BitSwarmEvent::IO_ERROR));
    boost::shared_ptr<map<string, boost::shared_ptr<void>>> args(
        new map<string, boost::shared_ptr<void>>());
